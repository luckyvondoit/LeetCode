...
//		dfs2(begin, visitor);
	...
	/** 非递归 */
	private void dfs1(V begin, VertexVisitor<V> visitor) {
		if (visitor == null) return;
	...
	/** 递归 */
	private void dfs2(V begin, VertexVisitor<V> visitor) {
		if (visitor == null) return;
		...
		Set<Vertex<V, E>> visitedVertices = new HashSet<>();
		dfs2(beginVertex, visitor, visitedVertices);
	}
...
//		if (visitor.stop) return;
		...
		for (Edge<V, E> edge : vertex.outEdges) {
			...
			if (visitedVertices.contains(edge.to)) continue;
			dfs2(edge.to, visitor,visitedVertices);
		}
	...
	@Override
	public Set<EdgeInfo<V, E>> mst(MST mst) {
		return switch (mst) {
			case prim -> prim();
